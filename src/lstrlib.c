...
static int str_dump(lua_State *L) {
  ...
  luaL_argcheck(L, lua_type(L, 1) == LUA_TFUNCTION && !lua_iscfunction(L, 1), 1,
  ...
  if (i >= ms->level) {
    ...
    *cap = ms->capture[i].init;
    if (l_unlikely(capl == CAP_UNFINISHED))
      ...
      lua_pushinteger(ms->L, ct_diff2S(ms->capture[i].init - ms->src_init) + 1);
    return capl;
  }
}
...
*/
static void packint(luaL_Buffer *b, lua_Unsigned n, int islittle, unsigned size,
                    int neg) {
...
*/
static void copywithendian(char *dest, const char *src, unsigned size,
                           int islittle) {
...
static int str_packsize(lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    luaL_argcheck(L, totalsize <= LUA_MAXINTEGER - size, 1,
...
static int str_unpack(lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    luaL_argcheck(L, ntoalign + size <= ld - pos, 2, "data string too short");
    ...
    case Kuint: {
      lua_Integer res =
          unpackint(L, data + pos, h.islittle, cast_int(size), (opt == Kint));
    ...
    case Kstring: {
      lua_Unsigned len =
          (lua_Unsigned)unpackint(L, data + pos, h.islittle, cast_int(size), 0);
