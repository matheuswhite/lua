...
static int tpanic(lua_State *L) {
  ...
  return (
      badexit("PANIC: unprotected error in call to Lua API (%s)\n", msg, NULL),
...
static int testobjref1(global_State *g, GCObject *f, GCObject *t) {
  ...
  if (issweepphase(g))
    ...
    if ((getage(f) == G_OLD && isblack(f)) && !isold(t))
      return 0;
    if ((getage(f) == G_OLD1 || getage(f) == G_TOUCHED2) && getage(t) == G_NEW)
      return 0;
    return 1;
  }
}
...
static void printobj(global_State *g, GCObject *o) {
  printf("||%s(%p)-%c%c(%02X)||", ttypename(novariant(o->tt)), (void *)o,
         isdead(g, o) ? 'd'
         : isblack(o) ? 'b'
         : iswhite(o) ? 'w'
                      : 'g',
...
void lua_printobj(lua_State *L, struct GCObject *o) { printobj(G(L), o); }
...
void lua_printvalue(TValue *v) {
  ...
  case LUA_VLCF:
    ...
    break;
  case LUA_VCCL:
    printf("C closure: %p", clCvalue(v));
    break;
  case LUA_VLCL:
    printf("Lua function: %p", clLvalue(v));
    break;
  case LUA_VTHREAD:
    printf("thread: %p", thvalue(v));
    break;
  case LUA_VTABLE:
    printf("table: %p", hvalue(v));
    break;
  default:
    lua_assert(0);
  }
...
static void checkobject(global_State *g, GCObject *o, int maybedead,
  ...
  if (isdead(g, o))
    ...
    if (g->gckind == KGC_GENMINOR) { /* generational mode? */
      ...
      if (isold(o)) {
        assert(!iswhite(o));
        assert(isblack(o) || getage(o) == G_TOUCHED1 || getage(o) == G_OLD0 ||
               o->tt == LUA_VTHREAD ||
...
static l_mem checkgraylist(global_State *g, GCObject *o) {
  ...
  while (o) {
    ...
    case LUA_VUSERDATA:
      ...
      break;
    default:
      assert(0); /* other objects cannot be in a gray list */
    }
  }
  return total;
}
...
static l_mem checklist(global_State *g, int maybedead, int tof, GCObject *newl,
                       GCObject *survival, GCObject *old, GCObject *reallyold) {
  GCObject *o;
...
static char *buildop(Proto *p, int pc, char *buff) {
  ...
  switch (getOpMode(o)) {
  case iABC:
    sprintf(buff, "%-12s%4d %4d %4d%s", name, GETARG_A(i), GETARG_B(i),
            GETARG_C(i), GETARG_k(i) ? " (k)" : "");
    break;
  case ivABC:
    sprintf(buff, "%-12s%4d %4d %4d%s", name, GETARG_A(i), GETARG_vB(i),
            GETARG_vC(i), GETARG_k(i) ? " (k)" : "");
    break;
  case iABx:
...
static int table_query(lua_State *L) {
  ...
  if (i == -1) {
    ...
    if (!isempty(gval(gnode(t, i))) || ttisnil(&k) || ttisnumber(&k)) {
      pushobject(L, &k);
    } else
...
static int runC(lua_State *L, lua_State *L1, const char *pc) {
  ...
  for (;;) {
    ...
    if EQ ("")
      ...
      int b = getnum;
      int c = getnum;
      luaL_gsub(L1, lua_tostring(L1, a), lua_tostring(L1, b),
                lua_tostring(L1, c));
    } else if EQ ("insert") {
