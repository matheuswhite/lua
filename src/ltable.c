...
static unsigned computesizes(Counters *ct) {
  ...
  for (i = 0, twotoi = 1; twotoi > 0 && arrayXhash(twotoi, ct->na);
       i++, twotoi *= 2) {
...
*/
static Value *resizearray(lua_State *L, Table *t, unsigned oldasize,
                          unsigned newasize) {
  if (oldasize == newasize)
    ...
    Value *np =
        cast(Value *, luaM_reallocvector(L, NULL, 0, newasizeb, lu_byte));
...
*/
static void reinsertOldSlice(Table *t, unsigned oldasize, unsigned newasize) {
  unsigned i;
