...
static void savelineinfo(FuncState *fs, Proto *f, int line) {
  ...
  luaM_growvector(fs->ls->L, f->lineinfo, pc, f->sizelineinfo, ls_byte, INT_MAX,
                  "opcodes");
...
int luaK_code(FuncState *fs, Instruction i) {
  ...
  luaM_growvector(fs->ls->L, f->code, fs->pc, f->sizecode, Instruction, INT_MAX,
                  "opcodes");
  f->code[fs->pc++] = i;
...
int luaK_codevABCk(FuncState *fs, OpCode o, int A, int B, int C, int k) {
  ...
  lua_assert(A <= MAXARG_A && B <= MAXARG_vB && C <= MAXARG_vC &&
             (k & ~1) == 0);
...
*/
static int isKint(expdesc *e) { return (e->k == VKINT && !hasjumps(e)); }
...
*/
static int isSCint(expdesc *e) { return isKint(e) && fitsC(e->u.ival); }
